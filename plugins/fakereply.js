const {
    ...
    prefix,
     ...
     smd({
        ...
        usage:"generates savage messages of given text and number!",
        filename: __filename,
        public : true,
      },
      async (m,text) => {
      ...
      let types = ["text","order","contact","image" , "video"]
          let args = text.split("|")
          if(!text || args.length < 3) return await m.reply(`*Use ${prefix}savagereply text |Reply_text|2348039607375|type(text,order,contact,image,video)*`)
          ...
          let  fak =await m.bot.savageMessage(type,{id: smds,remoteJid:m.isGroup? m.chat : num,participant:num},msg) 
